...
cat /var/log/apache2/access.log /var/log/apache2/access.log.1 > access.txt
logFile="access.txt"
...
function frequentVisitors(){
	local jawn=$(histogram)
	:> frequent.txt
	echo "$jawn" | while read -r line;
	do
		#echo $line
		one=$(echo $line | cut -d ' ' -f 1)
		if [ $one -gt 10 ]; then
			echo $line >> frequent.txt
		fi
	done
	cat frequent.txt
}
...
# it is welcomed.
function suspiciousVisitors(){
	cat access.txt | egrep -i -f ioc.txt | cut -d ' ' -f 1 | uniq -c
}
