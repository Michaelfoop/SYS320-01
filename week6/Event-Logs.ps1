...
function badGuys($blud){
    $fail = getFailedLogins $blud

    Write-Host($fail | Group-Object -Property user | Where-Object {$_.Count -gt 9} | Format-Table -Property count, name | out-string)
}
